...
{ /* shorten beat by 80% +/- 10% randomly chosen */
  ...
  delta = 0.2*((float)rand()/RAND_MAX - 0.5);
...
void initialize(long seed, long vector_length, float Pe, float Pn,
                float *changepoints, float *sleepstates)
{   
    ...
    /* set size in seconds and intialize global vector of r peaks */
    N = vector_length;
    ...
    /* define mean RR-interval using circadian rhythm period */
    ...
    for(i=1;i<=N;i++)
        RR0[i] += 0.2*RRamp * gasdev(&rseed);
    
    /* initialize count in time and beats */
    time = 1;
   ...
   while(time < N)
   {
	...
	/* generate RR during transition between blocks with rr-intervals rra, rrb */
	rrtrend = 0.0;
	...
	for(j=1;j<=Nt;j++)
            trpeaks[i+j] = trpeaks[i] + tr1[j];
	    i += Nt;

	for(j=1;j<=Nb;j++)
            trpeaks[i+j] = trpeaks[i] + tr0[j];
	    i += Nb;
	...
	time = (long)(trpeaks[i]);
        ...
        // Index, state change, sleep state
        /*
        printf("%li,%1.0f,%1.0f\n",
                i-Nt-Nb,
                changepoints[counter],
                sleepstates[counter]);
        */
  ...
  // Here we output RR time series before noise is inserted so that we can convert noise
  // indexes to time	
	int a;
	for(a=1; a<=Nbeats;a++){
		printf("%lf, %d, %d\n",trpeaks[a],7,7);
...
{    
    ...
    long vector_length = 24*60*60;   /* 24 hours, in seconds */
    ...
    // Convert string input argument into long
    seed = atol(argv[1]);
    ...
    */
    ...
    // Convert input arguments to floats for Pe and Pn
    if(argc>3) Pe = atof(argv[3]);
    if(argc>4) Pn = atof(argv[4]);

    // Allocate memory for state change vector
    float *changepoints = vector(1,vector_length); // Index in RR where state change occurr
    float *sleepstates = vector(1,vector_length); // Sleep state (1==awake,0==otherwise)

    // Initialize values of these vectors to 0
    int i;
    for (i=1; i<vector_length+1; i++)
    {
        changepoints[i] = -1; // means no state change
        sleepstates[i] = -1; // means no state change
    }

    // initialize and run RR interval generator, pass in length
    // and probability of artifacts and noise
    // This also generates change points
    initialize(seed, vector_length, Pe, Pn, 
               changepoints, sleepstates);

    // Initialize counter for RR interval index
    i = 1;

    // generate RR interval
    while (i < vector_length+1) {	/* add RR interval to running time */	
        ...
        // Loop through state change vectors 
        //for(int jj = 0; i<counter; j++)
        
        // Print 1) index and 2) RR interval value to screen
        printf("%5.3f,%1.0f,%1.0f\n",
                (ts - tsp)/((float)SPS),
                changepoints[i],
                sleepstates[i]); 
